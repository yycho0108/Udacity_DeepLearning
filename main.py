...
with graph.as_default():
  ...
  optimizer = tf.train.AdadeltaOptimizer(learning_rate = 0.05, rho=0.9, epsilon=1e-7).minimize(loss)
...
with tf.Session(graph=graph) as session:
    ...
    for step in range(num_steps):
        indices = np.random.randint(train_labels.shape[0],size=batch_size)
